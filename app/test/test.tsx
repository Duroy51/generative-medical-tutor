...
export default function HomePage() {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        const [casesRes, simulationsRes] = await Promise.all([
          ApiService.getClinicalCases(),
          ApiService.getSimulations(),
        ]);

        setCasesCount(casesRes.data.length);

        const inProgress = simulationsRes.data.filter(
          (s: any) => s.status === 'in_progress'
        );

        setActiveSimulations(inProgress.length);
      } catch (error) {
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="border rounded-lg p-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-gray-500">Cas cliniques</p>
            <p className="text-xl font-bold">{casesCount}</p>
          </div>
        </div>
        ...
        <div className="border rounded-lg p-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-gray-500">Simulations en cours</p>
            <p className="text-xl font-bold">{activeSimulations}</p>
          </div>
        </div>
      </div>
